...
const Login = () =>{
    ...
    return ( 
        ...
        <form className='absolute p-12 bg-black w-3/12 my-40 mx-auto right-0 left-0 text-white bg-opacity-80 rounded-lg'>
            <h1 className='font-bold text-3xl py-4'>{isSignInForm? "Sign In" : "Sign Up"}</h1>
            { !isSignInForm && <input type='text' placeholder='Full Name' className='p-4 my-2 w-full bg-gray-800 rounded-md' /> }

            <input type='text' placeholder='Email Address' className='p-4 my-2 w-full bg-gray-800 rounded-md' />
            <input type='password' placeholder='Password' className='p-4 my-2 w-full bg-gray-800  rounded-md' />
            <button className='p-4 my-10 bg-red-700 w-full  rounded-md'>{isSignInForm? "Sign In" : "Sign Up"}</button>
            <p className='py-4 cursor-pointer' onClick={toggleSignInForm}>
            {isSignInForm? "New to Netflix? Sign Up Now" : "User Already Exists!"}
            </p>
        </form>
        </>
    )
